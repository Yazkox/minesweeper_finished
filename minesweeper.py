...
color = [(13,143,214),(0,200,200),(100,190,70),(220,160,0),(240,100,80),(250,50,40),(255,20,20)]
...
class Selection:
    ...
    def update(self, coord):
        pygame.draw.rect(game_window, (175, 201, 26), (((540 // 2), self.hauteur), (500, 75)))

        if (540 / 2) <= coord[0] <= (540 / 2) + 500 and self.hauteur <= coord[1] <= self.hauteur + 75:
            pygame.draw.rect(game_window, (175, 201, 26), (((540 // 2), self.hauteur), (500, 75)), 10)
    ...
    for y in range(len(grid.display)):
        for x in range(len(grid.display[0])) :
            ...
            if grid.display[y][x].covered:
                if not grid.display[y][x].flagged:
                ...
                if grid.display[y][x].mined :
                    ...
                    if number >0 :
                        display_number = number_font.render(str(number), True, color[number-1])
                        game_window.blit(display_number, (x_pos+(frame_size-display_number.get_width())//2,y_pos+(frame_size-display_number.get_height())//2))
def on_click():
...
def start_game(attributes):
    ...
    if game_grid.lost:
        ...
        pygame.draw.rect(game_window, (175, 201, 26), ((20,300), (250, 150)))
        game_window.blit(death_message, (30, 355))
        for line in game_grid.display:
            for frame in line :
                if frame.mined:
                    frame._uncover()
        display_grid(game_grid)
        ...
        time_won = time.time()
        timer = time_won-starting_time
        win_message = title_font.render("GG, vous avez gagné en :" + str(timer) + "s", True, (0, 0, 0))
        pygame.draw.rect(game_window, (175, 201, 26), ((20,300), (250, 150)))
        game_window.blit(win_message, (30, 355))
    pygame.display.flip()
    while True :
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
